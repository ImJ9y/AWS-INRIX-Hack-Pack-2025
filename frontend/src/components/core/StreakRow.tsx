...
export function StreakRow({ streakLength, mondayStart = true, activity = defaultActivity }: StreakRowProps) {
  const days = mondayStart ? ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"] : ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
  const weeks: number[][] = [];
  ...
  return (
    <Card className="flex flex-col gap-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-medium uppercase tracking-[0.2em] text-ink-muted">Streak</p>
          <p className="text-2xl font-semibold leading-tight text-ink">Day {streakLength} streak</p>
          <p className="text-sm text-ink-muted">Consistent motion monitoring keeps the model sharp.</p>
        </div>
