...
class AWSInfrastructureSetup:
    ...
    def attach_thing_principal(self, thing_name: str, certificate_arn: str) -> bool:
        ...
        try:
            self.iot.attach_thing_principal(thingName=thing_name, principal=certificate_arn)
    ...
    def setup_sns_subscriptions(self, topic_arns: Dict[str, str]) -> bool:
        ...
        try:
            ...
            print("Note: You'll need to manually configure email and SMS subscriptions in the AWS Console")
            print("Topic ARNs:")
    ...
    def update_config_file(self, stack_outputs: Dict[str, str], 
                          certificate_info: Dict[str, str], 
                          iot_endpoint: str) -> bool:
        ...
        try:
            ...
            aws_config['iot_core']['certificate_path'] = certificate_info.get('certificate_file', '')
            aws_config['iot_core']['private_key_path'] = certificate_info.get('private_key_file', '')
            aws_config['iot_core']['root_ca_path'] = 'certs/AmazonRootCA1.pem'
            
            # Update SNS topic ARNs
            aws_config['sns']['emergency_topic_arn'] = stack_outputs.get('EmergencyTopicArn', '')
            aws_config['sns']['notification_topic_arn'] = stack_outputs.get('NotificationTopicArn', '')
            
            # Update S3 bucket name
            aws_config['s3']['bucket_name'] = stack_outputs.get('EmergencyDataBucketName', '')
            
            # Update DynamoDB table names
            aws_config['dynamodb']['table_name'] = stack_outputs.get('EventsTableName', '')
            aws_config['dynamodb']['analytics_table_name'] = stack_outputs.get('AnalyticsTableName', '')
            
            # Update Lambda function names
            aws_config['lambda']['emergency_processor_function'] = stack_outputs.get('EmergencyProcessorFunctionName', '')
            aws_config['lambda']['analytics_function'] = stack_outputs.get('AnalyticsProcessorFunctionName', '')
            
            # Update CloudWatch log group
            aws_config['cloudwatch']['log_group'] = stack_outputs.get('CloudWatchLogGroupName', '')
            ...
            env_updates = {
                ...
                'AWS_IOT_PRIVATE_KEY_PATH': certificate_info.get('private_key_file', ''),
                'AWS_IOT_ROOT_CA_PATH': 'certs/AmazonRootCA1.pem',
                'AWS_SNS_EMERGENCY_TOPIC_ARN': stack_outputs.get('EmergencyTopicArn', ''),
                'AWS_SNS_NOTIFICATION_TOPIC_ARN': stack_outputs.get('NotificationTopicArn', ''),
                'AWS_S3_BUCKET_NAME': stack_outputs.get('EmergencyDataBucketName', ''),
                'AWS_DYNAMODB_EVENTS_TABLE': stack_outputs.get('EventsTableName', ''),
                'AWS_DYNAMODB_ANALYTICS_TABLE': stack_outputs.get('AnalyticsTableName', ''),
                'AWS_LAMBDA_EMERGENCY_PROCESSOR': stack_outputs.get('EmergencyProcessorFunctionName', ''),
                'AWS_LAMBDA_ANALYTICS_PROCESSOR': stack_outputs.get('AnalyticsProcessorFunctionName', ''),
                'AWS_CLOUDWATCH_LOG_GROUP': stack_outputs.get('CloudWatchLogGroupName', '')
            }
...
def main():
    parser = argparse.ArgumentParser(description='Setup AWS infrastructure for Fall Detection System')
    ...
    parser.add_argument('--thing-name', default='fall-detection-camera', help='IoT thing name')
    parser.add_argument('--skip-cloudformation', action='store_true', 
    ...
    print(f"Setting up AWS infrastructure for {args.project_name} in {args.environment} environment")
    ...
    if not args.skip_iot:
        ...
        if not setup.attach_iot_policy(certificate_info['certificate_arn'], policy_name):
            print("Failed to attach IoT policy")
            sys.exit(1)
        
        # Attach certificate to thing
        if not setup.attach_thing_principal(args.thing_name, certificate_info['certificate_arn']):
