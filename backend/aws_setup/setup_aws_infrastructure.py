...
class AWSInfrastructureSetup:
    ...
    env_updates = {
        ...
        'AWS_IOT_CERT_PATH': certificate_info.get(
            'certificate_file',
            ''),
        ...
        'AWS_SNS_NOTIFICATION_TOPIC_ARN': stack_outputs.get(
            'NotificationTopicArn',
            ''),
        'AWS_S3_BUCKET_NAME': stack_outputs.get(
            'EmergencyDataBucketName',
            ''),
        'AWS_DYNAMODB_EVENTS_TABLE': stack_outputs.get(
            'EventsTableName',
            ''),
...
def main():
    ...
    print(
        f"Setting up AWS infrastructure for {
            args.project_name} in {
    ...
    if not setup.deploy_cloudformation_stack(
    ...
    if not setup.attach_iot_policy(
            certificate_info['certificate_arn'],
            policy_name):
