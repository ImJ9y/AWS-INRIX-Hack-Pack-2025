...
def process_analytics_data(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    fall_rate = (
        fall_count /
        total_detections *
        100) if total_detections > 0 else 0.0
    emergency_rate = (
        emergency_count /
        total_detections *
        100) if total_detections > 0 else 0.0
...
def update_reporting_tables(
    ...
    insights_table_name = os.getenv(
        'AWS_DYNAMODB_INSIGHTS_TABLE',
        'FallDetectionInsights')
